...
Java_com_sanad_full_whisper_WhisperLib_transcribe(JNIEnv *env, jclass clazz, jlong ptr, jint threads, jfloatArray audioData, jstring languageStr) {
    (void)clazz;
    ...
    p.translate = false;
    const char *language = languageStr ? (*env)->GetStringUTFChars(env, languageStr, NULL) : NULL;
    p.language = (language && language[0]) ? language : "ar";
