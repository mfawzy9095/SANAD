...
public class MainActivity extends Activity {
    ...
    public final class Bridge {
        ...
        @JavascriptInterface public void startVoice(String locale){
            voiceLocale=(locale==null||locale.isEmpty())?"ar-EG":locale;
            voiceLocaleIndex=0; voiceRetry=0; voiceDisconnectRetry=0; voiceBusyRetry=0; voiceRetrying=false;
            voiceSessionActive=true; voiceCycleActive=false; voiceSessionStartedAt=System.currentTimeMillis();
            runOnUiThread(MainActivity.this::resolveVoiceLocaleAndStart);
        }
        @JavascriptInterface public void stopVoice(){ runOnUiThread(MainActivity.this::stopVoiceSession); }
    ...
    @Override public void onRequestPermissionsResult(int req,String[] perms,int[] grants){
        ...
        if(req==REQ_AUDIO){
            if(granted) resolveVoiceLocaleAndStart();
            else{
